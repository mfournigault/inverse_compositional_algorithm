...
def inverse_compositional_algorithm(I1, I2, p, transform_type, nx, ny, nz, TOL, verbose):
    """
    ...
    :param p: Parameters of the transform (output), a numpy array of shape (nparams,).
    ...
    :param nz: Number of channels of the images.
    :param TOL: Tolerance used for the convergence in the iterations.
    :param verbose: Enable verbose mode.
    """

   #TODO: remove the use of params nx, ny, nz and define them from the shape of I1 and I2
   #TODO: implement verification of the dimensions of I1 and I2, they should be the same
   #TODO: implement sanity check on the value of TOL, it should be positive and very small
   #TODO: define p within the function and return it as output
   ...
   #TODO: replace by the gradient function from scipy.ndimage 
    # Ix, Iy = cf.gradient(I1, nx, ny, nz)
...
def robust_inverse_compositional_algorithm(
    I1,    # first image
    I2,    # second image
    p,     # parameters of the transform (output)
...
def pyramidal_inverse_compositional_algorithm(
    ...
    for s in range(nscales-1, -1, -1):
        ...
        if robust_type == 'QUADRATIC':
            if verbose:
                print("(L2 norm)")
            inverse_compositional_algorithm(I1s[s], I2s[s], ps[s], transform_type, nx[s], ny[s], nzz, TOL, verbose)
        else:
            if verbose:
                print(f"(Robust error function {robust_type})")
            robust_inverse_compositional_algorithm(I1s[s], I2s[s], ps[s], transform_type, nx[s], ny[s], nzz, TOL, robust_type, lambda_, verbose)
        if s > 0:
