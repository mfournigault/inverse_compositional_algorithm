...
def get_pixel_value(img, x, y):
    # img : [batch, H, W, C]
    # x, y : coordonnées entières, mêmes dimensions [batch, ny, nx]
    batch_size = tf.shape(img)[0]
    batch_idx = tf.reshape(tf.range(batch_size), [batch_size, 1, 1])
    b = tf.tile(batch_idx, [1, tf.shape(x)[1], tf.shape(x)[2]]) #TODO: debug this
...
def bicubic_sampler(image, grid):
    ...
    # Convertir les listes en tenseur de forme [batch, newH, newW, 4]
    ...
    # Accumulation sur les 16 voisins
    for i in range(4):
        for j in range(4):
            ...
            w = weights_x[..., i] * weights_y[..., j]   # [batch, newH, newW]
...
class RobustInverseCompositional(Layer):
    ...
    def warp_image(self, I2, p):
        if self.transform_type in [tr.TransformType.TRANSLATION, tr.TransformType.EUCLIDEAN,
            ...
            def sample_mask(mask, grid_int):
                b = tf.reshape(tf.range(tf.shape(mask)[0]), [tf.shape(mask)[0], 1, 1])
                b = tf.tile(b, [1, tf.shape(grid_int)[1], tf.shape(grid_int)[2]])
                indices = tf.stack([b, grid_int[..., 0], grid_int[..., 1]], axis=-1)
    ...
    def transformed_grid(self, p):
        ...
        # # On construit la grille de coordonnées sous forme absolue en (y, x)
        # grid = tf.stack([Y_new, X_new], axis=-1)
        # return grid
        return transformed
    ...
    def robust_error(self, DI):
        ...
        rho_list = []
        for image in self.batch_size:
            rho = io.robust_error_function(DI, self.lambda_, self.robust_type)
